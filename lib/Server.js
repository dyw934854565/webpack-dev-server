...
class Server {
  constructor(compiler, options = {}, _log, app, listeningApp) {
  ...
  decorateApp(server) {
    if (!this.options.decorateMiddleware) {
      return server;
    }
    const decorateServer = {};
    const decorateMiddleware = this.options.decorateMiddleware;
    const decorateArg = function decorateArg(arg) {
      if (typeof arg === 'function') {
        return decorateMiddleware(arg);
      }
      if (Array.isArray(arg)) {
        return arg.map(decorateArg);
      }
      return arg;
    };
    const decorateFn = function decorateFn(fn, server) {
      return function name() {
        const args = Array.prototype.slice.call(arguments, 0).map(decorateArg);
        return fn.apply(server, args);
      };
    };
    const fns = ['all', 'use'];
    [].concat(methods, fns).forEach((method) => {
      decorateServer[method] = decorateFn(server[method], server);
    });
